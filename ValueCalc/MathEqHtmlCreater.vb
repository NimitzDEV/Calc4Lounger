...
Module MathEqHtmlCreater
    ...
    Public Sub outputHtmlFile()
        ...
        tmpStr &= "\right )"
        Print(1, "<img alt="""" src=""http://www.forkosh.com/mimetex.cgi?\overline{a}" & "=\sum_{a=" & returnCounter & "}^{a}" & "\frac{1}{" & returnCounter & "}" & tmpStr & "=" & v2 & """ />")
        Print(1, "<br>")
        Print(1, "<br>")
        Print(1, "<br>")
        Print(1, "求平均偏差")
        ...
        tmpStr = Microsoft.VisualBasic.Left(tmpStr, Len(tmpStr) - 1)
        tmpStr &= "\right ]"
        Print(1, "<img alt="""" src=""http://www.forkosh.com/mimetex.cgi?\Delta\overline{a}" & "=\sum_{a=" & returnCounter & "}^{a}" & "\frac{1}{" & returnCounter & "}" & tmpStr & "=" & v3 & """ />")
        Print(1, "<br>")
        FileClose(1)
    End Sub
End Module
